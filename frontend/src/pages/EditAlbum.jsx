...
const EditAlbum = () => {
    ...
    useEffect(() => {
        setLoading(true);
        axios.get(`http://localhost:4444/albums/${id}`)
        .then((response) => {
    ...
    const handleEditAlbum = () => {
        ...
        setLoading(true);
        axios
            .put(`http://localhost:4444/albums/${id}`, data)
            .then(() => {
                setLoading(false);
                enqueueSnackbar('Album Edited', { variant: 'success' });
                navigate('/');
            })
            .catch((error) => {
                setLoading(false);
                //alert('An error occured. Check console');
                enqueueSnackbar('Album Edit Error', { variant: 'error' });
                console.log(error);
            });
        };
    return(
        <div className='p-4'>
            <BackButton />
            <h1 className='text-3x1 my-4'>Edit Album</h1>
            {loading ? <Spinner /> : ''}
