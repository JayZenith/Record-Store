...
import { Album } from '../../../../backend/models/albumModel';

const AlbumSingleCard = ({album}) => {
    const [showModal, setShowModal] = useState(false);

    return(
        <div
                    
                    className='border-2 border-gray-500 rounded-1g px-4 py-2 m-4 relative hover:shadow-x1'
                >
                    <h2 className='absolute top-1 right-2 px-4 py-1 bg-red-300 rounded-1g'>
                    {album.publishYear}
                    </h2>
                    <h4 className='my-2 text-gray-500'>{album.id}</h4>
                    ...
                    <div className='flex justify-start items-center gap-x-2'>
                        ...
                        <h2 className='my-1'>{album.author}</h2>
                    </div>
                    <div className='flex justify-between items-center gap-x-2 mt-4 p-4'>
                        <BiShow
                            className='test-3x1 text-blue-800 hover:text-black cursor-pointer'
                    ...
                    {
                        showModal && (
                            <AlbumModal album={album} onClose={() => setShowModal(false)} />
                        )
                    }
                </div>
    )
